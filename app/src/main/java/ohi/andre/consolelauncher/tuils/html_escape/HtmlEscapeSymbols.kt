...
import ohi.andre.consolelauncher.tuils.html_escape.Html4EscapeSymbolsInitializer.initializeHtml4
import ohi.andre.consolelauncher.tuils.html_escape.Html5EscapeSymbolsInitializer.initializeHtml5
 ...
 * @since 1.0.0
 */
internal class HtmlEscapeSymbols(references: References, escapeLevels: ByteArray) {
    /*
         ...
         */
    val NCRS_BY_CODEPOINT: ShortArray = ShortArray(NCRS_BY_CODEPOINT_LEN)
     ...
     */
    val NCRS_BY_CODEPOINT_OVERFLOW: MutableMap<Int?, Short?>? // No need to instantiate it until we know it's needed
     ...
     */
    val ESCAPE_LEVELS: ByteArray = ByteArray(MAX_ASCII_CHAR.code + 2)
     ...
     */
    val SORTED_NCRS: Array<CharArray?>
     ...
     */
    val DOUBLE_CODEPOINTS: Array<IntArray?>?
    ...
    init {
        ...
        val ncrs: MutableList<CharArray?> = ArrayList<CharArray?>(references.references.size + 5)
        val codepoints: MutableList<Int?> = ArrayList<Int?>(references.references.size + 5)
        val doubleCodepoints: MutableList<IntArray?> = ArrayList<IntArray?>(100)
        val ncrsByCodepointOverflow: MutableMap<Int?, Short?> = HashMap<Int?, Short?>(20)

        // For each reference, initialize its corresponding codepoint -> ncr and ncr -> codepoint structures
        for (reference in references.references) {
            val referenceNcr = reference.ncr
            val referenceCodepoints = reference.codepoints

            ncrs.add(referenceNcr)

            if (referenceCodepoints.size == 1) {
                // Only one codepoint (might be > 1 chars, though), this is the normal case

                val referenceCodepoint = referenceCodepoints[0]
                codepoints.add(referenceCodepoint)
            } else if (referenceCodepoints.size == 2) {
                ...
                throw RuntimeException(
                    "Unsupported codepoints #: " + referenceCodepoints.size + " for " + kotlin.text.String(
                        referenceNcr
                    )
                )
            }
        ...
        SORTED_NCRS = arrayOfNulls<CharArray>(ncrs.size)
        SORTED_CODEPOINTS = IntArray(codepoints.size)

        val ncrsOrdered: MutableList<CharArray?> = ArrayList<CharArray?>(ncrs)
        Collections.sort<CharArray?>(
            ncrsOrdered,
            Comparator { o1: CharArray?, o2: CharArray? ->
                Companion.compare(
                    o1!!,
                    o2!!,
                    0,
        ...
        for (i in SORTED_NCRS.indices) {
            val ncr = ncrsOrdered.get(i.toInt())
            SORTED_NCRS[i.toInt()] = ncr

            for (j in SORTED_NCRS.indices) {
                if (ncr.contentEquals(ncrs.get(j.toInt()))) {
                    val cp: Int = codepoints.get(j.toInt())!!
                    SORTED_CODEPOINTS[i.toInt()] = cp
                    ...
                    if (cp > 0) {
                        ...
                        if (cp < NCRS_BY_CODEPOINT_LEN) {
                            ...
                            if (NCRS_BY_CODEPOINT[cp] == NO_NCR) {
                                ...
                                NCRS_BY_CODEPOINT[cp] = i.toShort()
                            } else {
                                val positionOfCurrent: Int =
                                    positionInList(ncrs, SORTED_NCRS[NCRS_BY_CODEPOINT[cp].toInt()])
                                val positionOfNew: Int = positionInList(ncrs, ncr)
                            ...
                            ncrsByCodepointOverflow.put(cp, i.toShort())
                        }
                    }
        ...
        if (doubleCodepoints.size > 0) {
            DOUBLE_CODEPOINTS = arrayOfNulls<IntArray>(doubleCodepoints.size)
            for (i in DOUBLE_CODEPOINTS.indices) {
    ...
    internal class References {
        val references: MutableList<Reference> = ArrayList<Reference>(200)

        fun addReference(codepoint: Int, ncr: String) {
            this.references.add(Reference(ncr, intArrayOf(codepoint)))
        }

        fun addReference(codepoint0: Int, codepoint1: Int, ncr: String) {
            this.references.add(Reference(ncr, intArrayOf(codepoint0, codepoint1)))
        }
    }


    class Reference(ncr: String, val codepoints: IntArray) {
        val ncr: CharArray
     ...
     * unescape operations.
     */
        val HTML4_SYMBOLS: HtmlEscapeSymbols
        val HTML5_SYMBOLS: HtmlEscapeSymbols


        init {
            HTML4_SYMBOLS = initializeHtml4()
            HTML5_SYMBOLS = initializeHtml5()
        }
     ...
     */
        private fun positionInList(list: MutableList<CharArray?>, element: CharArray?): Int {
            var i = 0
            for (e in list) {
                if (e.contentEquals(element)) {
                    return i
                }
                i++
            }
     ...
     * a ';' suffix.
     */
        private fun compare(ncr: CharArray, text: String, start: Int, end: Int): Int {
            val textLen = end - start
            val maxCommon = min(ncr.size, textLen)
            var i: Int
            // char 0 is discarded, will be & in both cases
            i = 1
            while (i < maxCommon) {
                val tc = text.get(start + i)
                if (ncr[i] < tc) {
                    if (tc == ';') {
                        return 1
                    }
        ...
        private fun compare(ncr: CharArray, text: CharArray, start: Int, end: Int): Int {
            val textLen = end - start
            val maxCommon = min(ncr.size, textLen)
            var i: Int
            // char 0 is discarded, will be & in both cases
     ...
     */
        fun binarySearch(
            values: Array<CharArray?>,
            text: String, start: Int, end: Int
        ): Int {
            var low = 0
            var high = values.size - 1

            var partialIndex = Int.Companion.MIN_VALUE
            var partialValue = Int.Companion.MIN_VALUE

            while (low <= high) {
                val mid = (low + high) ushr 1
                val midVal = values[mid]!!

                val cmp: Int = compare(midVal, text, start, end)
                ...
                if (cmp == -1) {
                    ...
                    // Partial match
                    low = mid + 1
                    if (partialIndex == Int.Companion.MIN_VALUE || partialValue < cmp) {
                        partialIndex = mid
                        partialValue =
            ...
            if (partialIndex != Int.Companion.MIN_VALUE) {
                // We have a partial result. We return the closest result index as negative + (-10)
                return (-1) * (partialIndex + 10)
            }

            return Int.Companion.MIN_VALUE // Not found!
        }

        fun binarySearch(
            values: Array<CharArray?>,
            text: CharArray, start: Int, end: Int
        ): Int {
            var low = 0
            var high = values.size - 1
